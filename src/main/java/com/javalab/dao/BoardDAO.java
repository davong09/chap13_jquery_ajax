...
public class BoardDAO {
	...
	public int insertBoard(BoardVO boardVO) {
		...
		try{
			...
			pstmt.setInt(5, nextVal);				// reply_group			
	...
	*/
	...
	public List<BoardVO> getBoardList(BoardVO boardVO){
		 ...
		 * [첫게시물과 끝 게시물 구하는 공식]
		 * 1. row_number() 함수 사용
		 * - 시작 게시물 번호 : (사용자가요청한페이지 -1) * 한페이지에보여줄 게시물수 + 1
		 * - 끝 게시물 번호 : 시작번호 + 한페이지에 보여줄 게시물 수 -1
		 * 2. Fetch ~ Next 구문 사용
		 * - 건너띌 게시물수 : (사용자가요청한페이지 -1) * 한페이지에보여줄 게시물수
		 * - 가져올 게시물수 : 한페이지에 보여줄 게시물 수
		 */
		// 건너띌 게시물수
...
//			sql.append("where a.row_num between ? and ?");
			...
			sql.append("select bno, title, member_id, reg_date, hit_no, reply_group, reply_order, reply_indent ");
			...
			while(rs.next()) {
                ...
                // 계층형 답변 게시판 필드
                board.setReplyGroup(rs.getInt("reply_group"));
                board.setReplyOrder(rs.getInt("reply_order"));
                board.setReplyIndent(rs.getInt("reply_indent"));
	...
	public BoardVO getBoard(int bno) {
		...
		try {
			...
			if(rs.next()){
				...
				boardVO.setReplyGroup(rs.getInt("reply_group"));	// 그룹번호
				boardVO.setReplyOrder(rs.getInt("reply_order"));	// 그룹내 순서
				boardVO.setReplyIndent(rs.getInt("reply_indent"));	// 들여쓰기
			}        
	    } catch (SQLException e) {
